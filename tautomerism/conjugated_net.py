...
class ConjugatedNet(nn.Module):
    ...
    def train_val_split(self, X1, X2, X, YT, YA, val_size=0.2, random_state=42):
        ...
        X1_train, X1_val, X2_train, X2_val, YT_train, YT_val = train_test_split(X1, X2, YT, test_size=val_size,
                                                                                random_state=random_state)

        X_train, X_val, YA_train, YA_val = train_test_split(X, YA, test_size=val_size, random_state=random_state)

        X1_train, X2_train, X_train, YT_train, YA_train = self.array_to_tensor(X1_train, X2_train, X_train, YT_train,
                                                                               YA_train)
