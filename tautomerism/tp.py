...
class ConjugatedNet(nn.Module):
    ...
    def get_batches(self, X1, X2, X, YT, YA, batch_size=16):
        ...
        batch_size = np.ceil(len(X) / n_mb)

        T_mb = DataLoader(TauMBSplitter(X1, X2, YT), batch_size=self.batch_size, shuffle=True)
    ...
    def array_to_tensor(self, X1, X2, X, YT, YA):
        ...
        X1 = torch.from_numpy(X1.astype('float64'))
        X2 = torch.from_numpy(X2.astype('float64'))
    ...
    def fit(self, X1, X2, X, YT, YA, n_epoch=100, batch_size=128, lr=0.001, weight_decay=0, verbose=False):
    ...
    def predict_constant(self, X1, X2):
        const = self.predict_acidity(X2) - self.predict_acidity(X1)
        return const
