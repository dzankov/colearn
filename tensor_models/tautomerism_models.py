...
class TensorTauCoopRidge:
    ...
    def prepare_input(self, X1, X2, X, yT, yK):
        ...
        X1 = torch.from_numpy(X1.astype('float64')).to('cuda')
        X2 = torch.from_numpy(X2.astype('float64')).to('cuda')
        X = torch.from_numpy(X.astype('float64')).to('cuda')
        yT = torch.from_numpy(yT.astype('float64')).to('cuda')
        yK = torch.from_numpy(yK.astype('float64')).to('cuda')
        I = torch.from_numpy(np.eye(X.shape[1]).astype('float64')).cuda()
            
        return X1, X2, X, yT, yK, I

    def fit (self, X1, X2, X, yT, yK):
    
        X1, X2, X, yT, yK, I = self.prepare_input(X1, X2, X, yT, yK)
        ...
        z2 = (self.alpha * (X2 - X1).transpose(0, 1).mm(yT) + 
               (1 - self.alpha) * X.transpose(0, 1).mm(yK))
